...
if [[ -n "${1}" ]]; then
  ...
  if [[ -n "${2}" ]]; then
    ...
    if [[ $(lsblk -dnpo name | grep "^${2}$") ]]; then
      DISK="${2}"
  ...
  select IMAGE_FILENAME in /mnt/storage/images/*.img.gz
  do
