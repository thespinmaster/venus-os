...
# Same approach for the raw PID 0xD8. This corresponds to a PID 0x18
	...
	def __init__(self, app: inetboxapp.InetboxApp, port: str, lin_debug: bool):
		...
		# ---- instance-local state (IMPORTANT FIX) ----
		self.ts_response_buffer: list[bytes] = []
		self.cpp_in_buffer = [b"" for _ in range(6)]
		...
		self.log.setLevel(logging.DEBUG if lin_debug else logging.INFO)
		self.log.info("LIN async initialized")
		if lin_debug:
	...
	def data_received(self, data: bytes):
		...
		if len(self.rx_buffer) > self.MAX_RX_BUFFER:
			self.log.warning("RX buffer overflow, resyncing")
	...
	def _process_rx(self):
		...
		self.status_monitor()  
		# New input process: idea is, nothing to forget. 
		# So there is a turing-machine nessecary. 
		# This read 1 byte and decide to switch in the next level or 
		# to throw the input away.
		...
		while len(self.rx_buffer) > 0:
			...
			line = b'\x00' + bytes([self.rx_buffer[0]])
	...
	# send requested answer to 0x3d -> 0x7d with parity) but only, if I have the need to answer
			...
			self.log.debug("Waiting for remaining 9 bytes of frame")
			...
			self.log.debug(f"Full frame received: {line.hex(' ')} ({len(line)} bytes)")
			deb=""
			for b in line:
					deb=deb+f"{b:02x} "
			...
			self.log.debug(f"in < {line.hex(" ")}")
			self.log.debug(f"Processing full frame: {line.hex(' ')}")
	#        if len(line) != 12:
	...
	# multi-frame receive for buffer download from CPplus
			...
			if (line[:4]==buf_trans_id) and (line[4] in range(0x21, 0x27)):
					self.log.debug(f"Multi-frame buffer segment detected: 0x{line[4]:02x}")
	...
	# attention: this are raw frames with checksum -> see specification for details
			...
			self.log.debug(f"Matched command in CMD_CTRL: {cmd}")
	...
	def _send_answer_str(self, data_str):
			self.log.debug("_send_answer_str")
	...
	def prepare_tl_str_response(self, message_str, info_str):
			self.log.debug(f"prepare_tl_str_response:{message_str},{info_str}")
	...
	def prepare_tl_response(self, messages):
			self.log.debug("prepare_tl_response")
			self.ts_response_buffer.extend([messages])


	def _answer_tl_request(self):
			self.log.debug("_answer_tl_request")
...
#            print()
	...
	def assemble_cpp_buffer(self):
			self.log.debug("assemble_cpp_buffer")
	...
	def generate_inet_upload(self, s, p):
			self.log.debug("generate_inet_upload")
