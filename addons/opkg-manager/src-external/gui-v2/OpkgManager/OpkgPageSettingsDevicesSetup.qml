...
Page {
	...
	property string selectedServiceTypePath: "TestService" //SCALPLE
	...
	VeQuickItem {
		id: devicesPath
		uid: Global.systemSettings.serviceUid + "/Settings/OpkgManager/DevicesList"
		onValueChanged: root.createDevicesModel()
	}
	...
	function createDevicesModel() {
		...
		var entries = devicesPath.value.split(",")
		for (var i = 0; i < entries.length; i++) {
			...
			values.push({
				display: pathName,
				value: Global.systemSettings.serviceUid + "/Settings/OpkgManager/Devices/" + pathName
			})
		}
	...
	BaseListView {
		...
		model: VisibleItemModel {
			...
			ListRadioButtonGroup {
				id: serialDevice
				//% "Serial Device"
				text: qsTrId("opkg_serial_device")
				// currentValue
				optionModel: root.servicesOptions
				//% "Select"
				defaultSecondaryText: qsTrId("opkg_select")
				currentIndex: {
