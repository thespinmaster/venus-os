...
static QString detectRunnerRoot()
{
    const QString envRoot =
        QProcessEnvironment::systemEnvironment().value("OPKG_RUNNER_ROOT").trimmed();
    if (!envRoot.isEmpty() && QFileInfo::exists(envRoot)) {
        qWarning() << "[OpkgBridge] detectRunnerRoot: using OPKG_RUNNER_ROOT:" << envRoot;
        return envRoot;
    }
    if (!envRoot.isEmpty()) {
        qWarning() << "[OpkgBridge] detectRunnerRoot: OPKG_RUNNER_ROOT set but path does not exist:" << envRoot;
    }

    const QString deployed = QStringLiteral("/data/opkg-manager/opkg-bridge");
    if (QFileInfo::exists(deployed)) {
        qWarning() << "[OpkgBridge] detectRunnerRoot: using deployed path:" << deployed;
        return deployed;
    }

    const QString dev = QStringLiteral("/data/dev/addons/opkg-manager/src/data/opkg-manager/opkg-bridge");
    if (QFileInfo::exists(dev)) {
        qWarning() << "[OpkgBridge] detectRunnerRoot: using dev path:" << dev;
        return dev;
    }

    qWarning() << "[OpkgBridge] detectRunnerRoot: no valid path found, defaulting to:" << deployed;
    return deployed; // sensible default for production
}
...
const QHash<QString, FamilySpec> &commandRegistry()
{
	static const QHash<QString, FamilySpec> registry = []() {
		...
		feed.actions.insert(QStringLiteral("list"),
					ActionSpec { QStringLiteral("/usr/bin/python3"),
						     QStringList { runnerPath(QStringLiteral("opkg_helpers.py")),
							     QStringLiteral("feed") } });
