...
class UsbScanner:
	...
	def _publish_discovered_device(self, line: str):
		...
		path = f"/Discovered/sid_{jsonPayload['hash']}"
		if path not in self._dbusservice:
				# must create the path then update the value else dbus-spy nore the gui see the value
				self._dbusservice.add_path(path, "", valuetype = dbus.String)

		self._dbusservice[path] = json.dumps(jsonPayload)
