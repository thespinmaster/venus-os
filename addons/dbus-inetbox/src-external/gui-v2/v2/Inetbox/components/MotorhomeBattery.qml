...
OverviewWidget {
	...
	readonly property bool demoMode: demoModeItem.valid && demoModeItem.value > 0
	...
	contentItem: ColumnLayout {
		...
		RowLayout {
			...
			ColumnLayout {
				...
				// Time To Go
				Label {
					text: Global.system.battery.timeToGo == 0
						? ""
						: Utils.secondsToString(Global.system.battery.timeToGo)
		...
		ColumnLayout {
			...
			// Temperature / Renewables
			RowLayout {
				QuantityLabel {
					value: root.demoMode ? 23 : Global.system.battery.temperature
					unit: Global.systemSettings.temperatureUnit
					unitColor: Theme.color_overviewPage_widget_battery_font_secondary
					font.pixelSize: root.secondaryFontSize
					alignment: Qt.AlignRight
					visible: root.demoMode ? true : !isNaN(Global.system.battery.temperature)
				}
			...
			// Volts / Amps / Power
			RowLayout {
				// Calculate whether voltage, current and power quantities fit on the footer together.
				readonly property bool _useSmallFont:
					!quantityLabelFits(batteryVoltageDisplay)
					|| !quantityLabelFits(batteryPowerDisplay)

				function quantityLabelFits(label) {
					return root.width / 2
						- 2 * Theme.geometry_overviewPage_widget_content_horizontalMargin
						> quantityLabelWidth(
							batteryCurrentDisplay.valueText,
							batteryCurrentDisplay.unitText
						) / 2
						+ quantityLabelWidth(
							label.valueText,
							label.unitText
				...
				QuantityLabel {
					...
					unitColor: Theme.color_overviewPage_widget_battery_font_secondary
					font.pixelSize: parent._useSmallFont
						? Theme.font_overviewPage_battery_small
						: Theme.font_overviewPage_battery_large
					alignment: Qt.AlignLeft
				}
				// Amps
				QuantityLabel {
